...
import de.uni.bielefeld.sc.hterhors.psink.scio.semanticmr.ietemplates.expgroup.templates.type_based.TB_ExGrBOWTemplate.BOWScope;
...
public class TB_ExGrBOWTemplate extends AbstractFeatureTemplate<BOWScope> {
	...
	public List<BOWScope> generateFactorScopes(State state) {
		...
		for (EntityTemplate experimentalGroup : super.<EntityTemplate>getPredictedAnnotations(state)) {
			...
			addSFSFactor(factors, state.getInstance(), expGroupBOW, experimentalGroup, SCIOSlotTypes.hasOrganismModel);
			addSFSFactor(factors, state.getInstance(), expGroupBOW, experimentalGroup, SCIOSlotTypes.hasInjuryModel);
			addMFSFactor(factors, state.getInstance(), expGroupBOW, experimentalGroup, SCIOSlotTypes.hasTreatmentType);
