...
public class ExperimentalGroupLatexTableGenerator {

	public static void main(String[] args) throws IOException {

		File f1File = new File("dissertation_results/experimental_group/evaluations/", "merged_cardinality_f1.csv");
		File precFile = new File("dissertation_results/experimental_group/evaluations/",
				"merged_cardinality_precision.csv");
		File recFile = new File("dissertation_results/experimental_group/evaluations/",
		...
		String name = "ExperimentalGroup";
		String t = "experimental_group";
	...
	private static void fillMap(List<String> F1, Map<String, Map<String, Double>> f1Map) {
		...
		for (String string : F1) {
			...
			if (!string.startsWith("EXP GROUP MACRO"))
				continue;

			String data[] = string.split("\t");

			String key = data[0].trim();
			if (key.equals("EXP GROUP MACRO COMPONENTS SCORE"))
				continue;
			if (key.equals("EXP GROUP MACRO  OVERALL SCORE"))
				key = "Overall";
			if (key.equals("EXP GROUP MACROCARDINALITY"))
				key = "Cardinality";
			if (key.equals("EXP GROUP MACRO: TREATMENT Vehicle"))
				key = "$\\rightarrow$ sub";
			if (key.equals("EXP GROUP MACRO: TREATMENT Non Vehicle"))
				key = "$\\rightarrow$ main";
			if (key.equals("EXP GROUP MACRO: TREATMENT BOTH"))
				key = "hasTreatment";
			if (key.equals("EXP GROUP MACRO: ORG MODEL"))
				key = "hasOrganismModel";
			if (key.equals("EXP GROUP MACRO: INJURY MODEL"))
				key = "hasInjuryModel";

			double gold = Double.parseDouble(data[1]);
			double pred = Double.parseDouble(data[2]);
			double goldCov = Double.parseDouble(data[3]);
