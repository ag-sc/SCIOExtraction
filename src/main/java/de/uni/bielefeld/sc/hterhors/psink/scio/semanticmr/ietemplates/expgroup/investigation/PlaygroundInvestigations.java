...
public class PlaygroundInvestigations {

	public static void main(String[] args) throws Exception {
		PlaygroundInvestigations pg = new PlaygroundInvestigations(
...
//		pg.playGround();
	...
	public PlaygroundInvestigations(File file) throws Exception {
...
//			if(!instance.getName().startsWith("N133"))
//				continue;
...
//								&& senIndex < keyPoint + 50) {
						...
						if (!cont) {
							System.out.println("OOB Discard: " + senIndex + "\t" + finding);
							continue;
						}

						String sentence = listToString(instance.getDocument().getSentenceByIndex(senIndex));
						if (etAlPattern.matcher(sentence).find()) {
							System.out.println("ETAL Discard: " + senIndex + "\t" + finding);
							continue;
						}

						for (String splits : finding.split("\\W")) {
						 ...
						 * HEURISTIC 1
						 */
						for (String split : finding.trim().split("\\Wor\\W")) {
							findings.add(split);
						 ...
						 */
						if (p.index != 3)
							for (String split : finding.trim().split("\\Wand\\W")) {
								findings.add(split);
								System.out.println(senIndex + "\t" + split);
							}
			...
			clusterFindings(countExpGroups.get(instance.getName()), findings);
...
//		countWords.entrySet().forEach(e -> System.out.println(e.getKey() + "\t" + e.getValue()));
	...
	private Integer getReferencePoint(Instance instance) {
		...
		for (DocumentToken docToken : instance.getDocument().tokenList) {
			...
			if (docToken.getText().toLowerCase().equals("references") && docToken.getSenTokenIndex() == 0)
				return docToken.getSentenceIndex();

		}

		/*
		 * If none found, infer with heuristics.
		 */
		List<Boolean> shortSentences = new ArrayList<>();
		for (List<DocumentToken> sentence : instance.getDocument().getSentences()) {
			String sen = listToString(sentence);

			float countP = 0;
			int countEvidence = 0;
			for (DocumentToken sentenceToken : sentence) {
				countP += sentenceToken.isPunctuation() ? 1 : 0;
				countEvidence += sentenceToken.getText().matches("\\d{4}") ? 2 : 0;
				countEvidence += sentenceToken.getText().equals(";") ? 1 : 0;
				countEvidence += sentenceToken.getText().equals("Neurosci") ? 10 : 0;
				countEvidence += sentenceToken.getText().equals("Neurol") ? 10 : 0;
				countEvidence += sentenceToken.getText().equals("Neurotrauma") ? 10 : 0;
				countEvidence += sentenceToken.getText().equals("Biotechnol") ? 10 : 0;
			}
			countEvidence += sen.matches("[A-Z] \\.") ? 4 : 0;
			countEvidence += sen.matches("\\d{2,4} ?. ?\\d{2,4}") ? 5 : 0;
			countEvidence += sen.contains("et al") ? 3 : 0;
			shortSentences.add(sen.length() < 80);

			/*
			 * HEURISTIC if more than 3 % of symbols are punctuations.
			 */
			if (100f / ((float) sen.length()) * countP > 3) {
				countEvidence += 4;
			}

			if (countEvidence > 15) {
				if (sentence.get(0).getSentenceIndex() > (instance.getDocument().getSentences().size() / 4)) {
					System.out.println("EV " + countP);
					return sentence.get(0).getSentenceIndex();
				}
			}

		}

		int countShortConseSen = 0;
		int sentenceIndex = 0;
		for (Boolean b : shortSentences) {
			if (b) {
				countShortConseSen++;
			} else {
				countShortConseSen = 0;
			}
			if (countShortConseSen == 5) {
				sentenceIndex -= 5;
				if (sentenceIndex > (instance.getDocument().getSentences().size() / 2)) {
	...
	private static final Set<String> SPECIAL_KEEPWORDS = new HashSet<>(Arrays.asList("control", "sham"));

	private static final Set<String> SPECIAL_STOPWORDS = new HashSet<>(
			Arrays.asList("cord", "spinal", "identifi", "cation", "n", "treated", "treatment", "", "received", "the",
					"injured", "transplanted", "all", "lesioned", "fi", "rst", "first", "second", "third", "fourth",
					"group", "animals", "rats", "in", "same", "individual", "groups", "were"));
	...
	static {
		...
		ALL_STOPWORDS.addAll(SPECIAL_STOPWORDS);
		ALL_STOPWORDS.removeAll(SPECIAL_KEEPWORDS);
	}

	private void clusterFindings(final int numOfClusters, List<String> findings) {

		HashMap<Integer, Set<String>> clusters = new HashMap<>();
		for (int i = 0; i < numOfClusters; i++)
			clusters.put(i, new HashSet<>());

		List<Set<String>> BOWFindings = new ArrayList<>();

		for (String finding : findings) {

			Set<String> tokens = new HashSet<>();
			for (String token : Arrays.asList(finding.split("\\W"))) {
			 ...
			 * HEURISTIC 5
			 */
			tokens.removeAll(ALL_STOPWORDS);
		...
		for (Set<String> set : BOWFindings) {
